---
description: Coordinate with another agent over the shared substrate: read others' live intent, declare your own, and use @mentions only for blocks and forks.
...
3. Lead with the substrate, not the message bus. Prefer the tools in this order:
   - read first: `read_intents_for_room` (what others are doing and which files they claim), then `mentions` (consume) to drain block/ask interrupts, before planning edits.
   - declare: `write_intent` (what you are doing, which files you claim), before you act.
...
Use `/harness` for full planning/execution. Use `/coordinate` only for cross-agent intent, messaging, and handoff. The default is to read and declare on the substrate; a mention is the exception, for blocks and forks.
