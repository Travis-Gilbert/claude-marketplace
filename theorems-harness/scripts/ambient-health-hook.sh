...
jq -n \
  ...
  '{
    ...
    hookSpecificOutput: {
      ...
      additionalContext: ("Harness ambient mode is degraded: " + $detail + ". Pending durable calls: " + $pending + ". The session remains usable; inspect scripts/ambient-status.sh for exact surfaces.")
    }
  }'
