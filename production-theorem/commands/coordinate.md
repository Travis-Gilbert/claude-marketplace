...
Run the production-theorem:orchestrate-coordinate skill against the user's message.
...
2. Invoke the `production-theorem:orchestrate-coordinate` skill with the full argument string.
...
Use `/orchestrate` for full planning/execution. Use `/coordinate` only for cross-agent presence, messaging, and handoff behavior.
