---
description: Execute work end-to-end through the production-theorem harness. Accepts a slug, a path to a plan-shaped file, or a plain task description.
argument-hint: <slug-or-path-or-task>
allowed-tools: Read, Write, Edit, Grep, Glob, LS, Bash, Agent
---

Execute the production-theorem harness on the user's task.
...
3. Invoke the `production-theorem:execute` skill via the Skill tool, passing the resolved input as the argument. The skill body (`skills/execute/SKILL.md`) is the authoritative spec for how to run the execution loop, write commits, run the per-stage review, and produce the Execute-Theorem Report.
