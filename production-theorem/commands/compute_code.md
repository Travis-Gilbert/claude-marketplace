...
Run the compute_code skill against the user's input.
...
2. Invoke the `compute_code` skill via the Skill tool, passing the user's argument as the input. The skill body (`skills/compute_code/SKILL.md`) is the authoritative spec for intent-to-algorithm routing.
