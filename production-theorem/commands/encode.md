...
Run the production-theorem:encode skill against the user's note.
...
2. Invoke the `production-theorem:encode` skill via the Skill tool, passing the user's full argument as input.
...
Agents may also invoke `production-theorem:encode` without an explicit slash command when a session produces a durable lesson, a notable success/failure, or a postmortem-worthy incident.
