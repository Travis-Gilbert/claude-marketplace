...
- `skills/harness_theorem/`, `skills/context-refresh/`, `skills/harness-coordinate/`, `skills/orchestrate/` (compatibility), `skills/planning-theorem/`, `skills/theorize/`, `skills/execute/`, `skills/encode/`
...
`production-theorem` packages Theorem's Harness, which is the user-visible
product layer. Slash commands and skills live here; the SDK and MCP surfaces sit
underneath it.

| Layer | Role |
|---|---|
| Slash commands | Human/agent entrypoints such as `/harness`, `/context-refresh`, `/coordinate`, `/encode`, and `/compute_code`; `/orchestrate` is compatibility |
...
belongs to this plugin's `/harness`.
