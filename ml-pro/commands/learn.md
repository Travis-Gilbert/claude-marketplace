...
{"event":"agent_invoked","agent":"training-engineer","trigger":"building training loop for GNN"}
{"event":"claim_consulted","claim_id":"claim-007","relevance_score":0.85}
{"event":"suggestion","suggestion_id":"sug-001","file":"train.py","claim_refs":["claim-007"]}
{"event":"suggestion_outcome","suggestion_id":"sug-001","outcome":"accepted"}
{"event":"candidate_claim","description":"AdamW with weight_decay=0.01 outperforms Adam for GNN link prediction tasks"}
{"event":"auto_capture","claims_added":["a1b2c3d4e5f6"],"solution_file":"knowledge/solutions/ml-training-2026-03-30.md","domain":"ml.training","project":"myproject"}
{"event":"session_end","timestamp":"...","duration_minutes":25,"files_changed":["train.py","model.py"]}
```
...
```bash
python -m scripts.epistemic.learn --plugin ml-pro
```
...
```
Tension detected:
  "claim-003 says always use cosine annealing scheduler, but this session
   used linear warmup + decay for the same training setup"
...
```
Observed pattern:
  "AdamW with weight_decay=0.01 outperforms Adam for GNN link prediction tasks"
...
Auto-captured this session:
  [1] "AdamW with weight_decay=0.01 outperforms Adam for GNN link prediction"
      Domain: ml.training | Confidence: 0.67
      Source: knowledge/solutions/ml-training-2026-03-30.md
...
```
Needs attention:
  claim-012 (confidence 0.38): "Always use gradient checkpointing for models > 1B params"
...
```
[ml-pro]: N active claims, avg confidence X.XX, M items reviewed
```

If there were no review items, just print:

```
[ml-pro]: N active claims, avg confidence X.XX, session saved
```
