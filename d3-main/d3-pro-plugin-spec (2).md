...
│   └── framework/                     # Observable Framework (selective)
│       ├── style/                     # CSS variable system + 13 named themes
│       ├── docs-lib-d3.md            # Canonical D3 integration example
│       └── grid.css                   # Dashboard grid layout system
│
...
| `/framework` | d3-architect | Target Observable Framework. Reactive Markdown, display(), CSS vars. |
...
| Property | Value |
|---|---|
| fill | `d3.schemeObservable10` (preferred), `schemeCategory10`, or `schemeTableau10` |
| stroke | `#fff` |
| stroke-width | 1.5 |
| r | 5 |
...
**Theme-aware alternative (preferred for new work):**

When targeting Observable Framework or any environment with its CSS
variables defined, use semantic tokens instead of hardcoded values:

```javascript
// Nodes
node.attr("stroke", "var(--theme-background)");   // auto-contrasts
node.attr("fill", d => color(d.group));            // color still from D3 scheme

// Links
link.attr("stroke", "var(--theme-foreground-faint)");  // 50% foreground
```

This produces identical results in light mode and automatically adapts
in dark mode. The hardcoded hex values above remain correct for
standalone use without Framework's CSS.
...
Uses Observable Framework's CSS variable system for automatic theming.
When targeting Framework, just set the `theme: dark` config option.
For standalone HTML, inline the variable definitions:
...
:root {
  ...
  --theme-background-alt: color-mix(in srgb, var(--theme-foreground) 4%, var(--theme-background));
}
```

D3 code using `var(--theme-*)` references works in both modes unchanged.
For named dark themes (deep-space, stark, midnight, etc.), swap in
Framework's theme file values. Node fills use `d3.schemeSet2` (higher
luminance than Category10 for dark backgrounds).
...
dashboards, and reports. It is built by the same team that created D3 and
represents their opinionated view of how D3 visualizations should be
authored, themed, and deployed. When a user is building a data dashboard
or analytical report, Framework is often the ideal deployment target.

### Why Framework Matters for D3-Pro

1. **It is the canonical D3 authoring environment.** The D3 examples on
   Observable use Framework's reactive Markdown model. Understanding this
   model means understanding how the D3 team intends their library to be
   used.

2. **The CSS variable system is the right way to theme D3.** Framework
   defines a semantic color system (`--theme-foreground-faint`,
   `--theme-background`, etc.) that D3 visualizations reference instead of
   hardcoded hex values. This gives you automatic light/dark mode switching
   with zero code changes.

3. **The dashboard grid system is production-ready.** Framework's `.grid`
   CSS classes handle responsive multi-chart layouts with container queries,
   so you can focus on the visualizations without writing layout CSS.

### The CSS Variable System

Framework's theme architecture defines two base colors and derives
everything else via `color-mix()`:
...
:root {
  ...
  --theme-foreground-focus: #3b5fc0;    /* links, interactive elements */
}

/* Derived palette (computed automatically) */
:root {
...
**Using these in D3 code:**

```javascript
// Instead of hardcoded colors:
link.attr("stroke", "#999");                    // BAD: breaks in dark mode

// Use Framework's semantic variables:
link.attr("stroke", "var(--theme-foreground-faint)");   // GOOD: auto-adapts
node.attr("stroke", "var(--theme-background)");          // GOOD: always contrast
```

The canonical Les Mis example in Framework uses exactly this pattern.
Node strokes use `var(--theme-background)` and link strokes use
`var(--theme-foreground-faint)`.

### Named Themes

Framework ships 13 named themes. Each one sets `--theme-foreground`,
`--theme-background`, and `--theme-foreground-focus`; the rest are derived.

| Theme | Background | Foreground | Character |
|---|---|---|---|
| (default light) | `#ffffff` | `#1b1e23` | Clean white |
| (default dark) | `#161616` | `#e0e0e0` | Neutral dark |
| deep-space | `#000000` | `#e6e6e6` | Pure black, purple focus |
| stark | `#000000` | `#ffffff` | Max contrast, yellow focus |
| midnight | dark | light | Deep blue-black |
| ink | dark | light | Warm dark |
| slate | dark | light | Gray-blue dark |
| ocean-floor | dark | light | Deep teal |
| coffee | light | dark | Warm tan |
| parchment | light | dark | Off-white, aged |
| cotton | light | dark | Soft white |
| glacier | light | dark | Cool blue-white |
| sun-faded | light | dark | Warm, muted |

When the `/style dark` preset is active, D3-Pro should output D3 code
that uses `var(--theme-*)` variables. If the target is Framework, the
theme handles everything. If the target is standalone HTML, D3-Pro should
inline the relevant `:root` definitions from Framework's theme files.

### Typography System

Framework's global CSS defines three font stacks that D3 labels and
annotations should reference:

```css
:root {
  --serif: "Source Serif 4", "Iowan Old Style", "Apple Garamond",
    "Palatino Linotype", "Times New Roman", serif;
  --sans-serif: -apple-system, BlinkMacSystemFont, "avenir next",
...
Axis labels and annotations should use `var(--sans-serif)`.
Data values in tooltips should use `var(--monospace)`.
Prose labels (chart titles, descriptions) should use `var(--serif)`.

### Reactive Markdown Authoring

In Framework, D3 code lives in fenced JavaScript blocks inside Markdown
files. The `display()` function renders a DOM node into the page:
...
- `d3` is available globally (no import needed)
- `display()` replaces `document.body.append()`
- `FileAttachment("data.json").json()` loads local data files
- Code blocks are reactive: changing a variable re-runs dependent blocks
- `width` is a built-in reactive variable tracking container width
- `Inputs.range()`, `Inputs.select()`, etc. provide interactive controls

### Dashboard Grid Layout

Framework provides a responsive grid system via CSS classes:

```md
<div class="grid grid-cols-2">
  <div class="card">${forceGraph}</div>
  <div class="card">${treemap}</div>
</div>

<div class="grid grid-cols-4">
  <div class="card grid-colspan-3">${mainChart}</div>
  <div class="card">${sidebar}</div>
</div>
```

Grid breakpoints use container queries:
- `grid-cols-2` and `grid-cols-4` activate at 640px container width
- `grid-cols-3` activates at 720px
- Full `grid-cols-4` at 1080px
- `grid-colspan-N` spans N columns
- `grid-rowspan-N` spans N rows

### What to Include in refs/framework/

Only include the parts that inform D3 visualization work:

```
refs/framework/
├── style/
│   ├── global.css              # Font stacks, base typography, semantic colors
│   ├── abstract-light.css      # Light theme derived palette
│   ├── abstract-dark.css       # Dark theme derived palette
│   ├── theme-deep-space.css    # Named dark theme example
│   ├── theme-stark.css         # Named dark theme example
│   ├── theme-parchment.css     # Named light theme example
│   ├── grid.css                # Dashboard grid layout
│   └── card.css                # Card component styling
├── docs-lib-d3.md              # Canonical D3 + Framework integration
└── README-excerpt.md           # Framework overview (trimmed to relevant parts)
```

Do NOT include: `src/build.ts`, `src/deploy.ts`, `src/loader.ts`,
`src/npm.ts`, data loader examples, or any SSG infrastructure. Those are
build system concerns, not D3 knowledge.
