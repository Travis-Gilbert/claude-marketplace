...
description: >-
  ...
  embeddings.py, or advanced_nlp.py.
...
**Entity types mapping to ObjectType:** spaCy's NER labels (PERSON, ORG, GPE, DATE, WORK_OF_ART, etc.) map to research_api ObjectType values. This mapping must be maintained in the NER extraction code. When adding new ObjectTypes, check if a spaCy entity label naturally maps to it.
...
- The research_api files listed above for existing patterns and the two-mode contract
