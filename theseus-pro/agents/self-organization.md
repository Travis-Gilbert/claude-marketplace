...
description: >-
  ...
  - <example>User asks "how do feedback loops connect to the Theseus levels?"</example>
model: inherit
color: magenta
tools:
  - Read
...
- **`community.py`**: Louvain community detection implementation. Wraps networkx community detection with Index-API-specific graph construction.

## Theseus Integration

The five feedback loops are the foundation for Levels 4-5 of the Theseus stack. Edge decay will eventually be replaced by learned temporal memory (Level 3) that captures non-exponential decay patterns from user behavior data. Emergent type detection connects to symbolic reasoning and schema induction (Level 4) -- when the system proposes a new ObjectType, symbolic reasoning validates whether the proposed type is logically consistent with the existing ontology. At Level 5, per-cluster engine configuration turns each community into a domain-specialized engine, with evolutionary optimization tuning the self-organization thresholds per cluster. The Organization axis of the IQ Tracker directly measures the quality of self-organization output.
...
Read the source before writing code. Read `self_organize.py` for the actual loop implementations and thresholds. Read `auto_classify.py` for the classification heuristics. Read `community.py` for the Louvain integration. Do not assume standard textbook implementations -- these have Index-API-specific adaptations.
