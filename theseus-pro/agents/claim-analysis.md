...
description: >-
  ...
  working on claim extraction, contradiction detection, NLI scoring, the
  epistemic status lifecycle, or Tension features for the IQ Tracker.
  ...
  - <example>User asks "build NLI features for the Level 2 learned scorer"</example>
model: inherit
color: yellow
tools:
  - Read
...
**Threshold calibration:** In Index-API, contradiction scores above 0.7 create Tension objects. Entailment scores above 0.8 create support edges. These thresholds were chosen empirically — lower thresholds produce too many false positives on heterogeneous corpora.
...
In Index-API, stance detection is implemented via NLI as a proxy: high entailment = support, high contradiction = opposition, high neutral = no clear stance.
...
NLI scores (entailment, contradiction, neutral) become features in the Level 2 learned scorer, letting the model learn which contradiction signals are real versus noise. Claim decomposition quality directly drives the Tension axis of the IQ Tracker. At Level 3, fine-tuned LMs produce higher-quality claim decomposition than rule-based fallbacks, closing the loop between language model training and claim analysis. At Level 7 (counterfactual simulation), claims are the atomic units that get retracted or upheld in truth maintenance systems -- the quality of decomposition determines the granularity of counterfactual reasoning.
...
- The Index-API files listed above for existing decomposition and NLI patterns
