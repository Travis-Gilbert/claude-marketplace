...
description: >-
  ...
  - <example>User asks "build a corpus for training the Level 2 scorer"</example>
model: inherit
color: cyan
tools:
  - Read
...
Web acquisition is the primary input pipeline for all Theseus levels. Corpus quality determines ceiling performance for every downstream model -- the Level 2 scorer, Level 3 GNN, and Level 3 fine-tuned LM all train on data that entered through this pipeline. At Level 5, domain-specialized engines may have per-cluster acquisition strategies: a science-focused cluster might prioritize ArXiv PDFs while a policy-focused cluster prioritizes government reports. The Ingestion axis of the IQ Tracker measures acquisition quality (source diversity, extraction completeness, failure rate). Multimodal acquisition (images via SAM-2, audio via Whisper) feeds into the multimodal networks agent at Level 3+.
