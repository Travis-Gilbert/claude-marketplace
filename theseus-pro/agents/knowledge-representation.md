...
description: >-
  ...
  with KGE training, modifying the promotion pipeline, reasoning about how
  knowledge should be structured, or producing KGE node features for GNN
  training.
  ...
  - <example>User asks "export KGE embeddings as GNN node features"</example>
model: inherit
color: magenta
tools:
  - Read
...
**Why RotatE for Index-API:** It naturally handles:
...
**ObjectType hierarchy:** The 10 object types in Index-API are not arbitrary categories — they represent fundamentally different epistemic roles:
...
KGE embeddings (RotatE, DE-SimplE) serve dual purpose: they predict missing links at Level 1 and become node features for GNN training at Level 3+. The 128-dimensional RotatE embedding is concatenated with SBERT embeddings and graph topology features to form the full node feature vector that R-GCN consumes. At Level 4 (emergent ontology), the promotion pipeline feeds into schema induction -- when enough entities of a proposed new type get promoted, the system considers formalizing the type. DE-SimplE temporal embeddings connect to the temporal graph memory agent for learning how entity relationships evolve over time.

## KGE RotatE Integration

RotatE knowledge graph embeddings provide structural link prediction. See
PATTERNS-kge-rotate.md for the full five-phase implementation.

### Triple Export (Phase K1)

Export edges as (head, relation, tail) triples in entity_to_id format.
Filter: only relations with >= 10 instances (sparse relations hurt
training). Materialize inverse triples for symmetric relations (similarity,
shared_entity, structural). Split 80/10/10 train/val/test.

### Entity Embeddings (Phase K2)

Train RotatE via PyKEEN on Modal GPU. Embedding dim 128 (matches GNN
output for alignment loss). Early stopping on validation MRR. Negative
sampling ratio 1:5.

### Relation Grammar

Index has 14 edge types, each with distinct relational behavior:
- Symmetric: similarity, shared_entity, structural (rotation = 0 or pi)
- Antisymmetric: entailment, causal (non-trivial rotation)
- Compositional: causal chains (rotation composition)

RotatE naturally models all three via complex-space rotations.

### Cold-Start Strategy

New Objects have no trained embedding. Cold-start prediction averages
the embeddings of known neighbors. If an Object has no graph neighbors,
KGE returns no predictions (graceful empty response, not errors).

### Structural Token Generation (Phase K5)

Generate bracket-format tokens for GL-Fusion Stream C:
`[KGE supports:ConceptName score:0.85]`. Budget: 150 tokens. Pre-compute
before the LLM generation loop, never during.
...
- The Index-API `models.py` for the current data model
- The Index-API `scripts/train_kge.py` for the training pipeline
